...
```
- Map은 인터페이스이고, HashMap은 이를 구현한 클래스이며, 종류가 다양하지만 가장 기본적인 형태가 HashMap이다.

- 위 메서드에서 사용한 Map 조건은 Key는 문자열 형태인 데이터를 찾고, 그 데이터는 어떤 값이든(Object : 자바 최상위 클래스) 가능하다. 

- 또한 제네릭은 오직 객체만 받기 때문에, Value에는 래퍼 클래스만 작성할 수 있다.

아래는 위의 에시를 그림으로 나타낸 형태이다.
...
1. Key 값은 중복될 수 없다.
2. Value는 중복될 수 있다.
3. 순서를 보장하지 않는다. (LinkedHashMap은 보장함)
